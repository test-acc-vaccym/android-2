...
import static com.edroplet.sanetel.utils.ImageUtil.dip2px;

/**
 * Created by qxs on 2017/9/16.
 */
...
public class CustomRadioGroup extends RadioGroup {
    ...
    public CustomRadioGroup(Context context) { this(context, null); }
    ...
    //设置水平间距（单位dp）
    public void setHorizontalSpacing(int horizontalSpacing_dp) { this.horizontalSpacing = dip2px(mContext, horizontalSpacing_dp); }

    //设置垂直间距（单位dp）
    public void setVerticalSpacing(int verticalSpacing_dp) { this.verticalSpacing = dip2px(mContext, verticalSpacing_dp); ; }
    ...
    class RowView {
        ...
        public RowView() { lineViews = new ArrayList<>(); }
        public List<View> getRowViews() { return lineViews; }
        ...
        public void addChidView(View view) {
            if (!lineViews.contains(view)){
                ((com.edroplet.sanetel.view.custom.CustomRadioButton)view).setOnClickListener(new OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        if(listener!=null){
                            listener.OnText(((com.edroplet.sanetel.view.custom.CustomRadioButton)v).getText().toString().trim());
                        }
                    }
                });
                //更新高度和宽度
                if (lineViews.size() == 0) { //第一次添加view,不用添加水平间距
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec){
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (rowView == null) { rowView = new RowView(); }
            if (rowView.getRowViews().size() == 0) {
                //如果当前行一个view都没有，，直接添加，不用比较
        ...
        //计算layout所有行需要的高度
        int height = getPaddingTop() + getPaddingBottom();//加上padding值
        for (int i = 0; i < rowViews.size(); i++){
            height += rowViews.get(i).getRowHeight();//添加每行高度
        }
        height += (rowViews.size() - 1) * verticalSpacing;//添加垂直间距高度
        setMeasuredDimension(width, height);//向父view申请宽带和高度
        if (getChildCount() == 0) {
