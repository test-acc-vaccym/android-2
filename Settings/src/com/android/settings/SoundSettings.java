...
public class SoundSettings extends SettingsPreferenceFragment implements Indexable {
    ...
    private static final boolean DOLBY_ALLOW_PROFILE_SELECTION = true;
    // DOLBY_DAP_GUI END
    ...
    private final IDsClientEvents mDsListener = new IDsClientEvents() {
        ...
        public void onClientConnected() {
			Log.d(TAG, "zhaodong add, IDsClientEvents onClientConnected()");
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // DOLBY begin
        final PreferenceCategory dolby = (PreferenceCategory) findPreference(KEY_DOLBY);
        // DOLBY end
    ...
    private void initDolby(PreferenceCategory root) {
        mDolbyLaunchApp = root.findPreference(KEY_DOLBY_DDP);
        mDolbyLaunchApp.setEnabled(false);
        mDolbyLaunchApp.setOnPreferenceClickListener(new OnPreferenceClickListener() {
            @Override
        ...
        mDolbyLaunchApp.setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
            @Override
            public boolean onPreferenceChange(Preference preference, Object newValue) {
                final boolean val = (Boolean) newValue;
                if (mDsClientConnected) {
                    try {
                        mDsClient.setDsOn(val);
                        updateDolbyStateUI();
                    } catch (Exception e) {
                        e.printStackTrace();
                        unbindDsClient();
        ...
        mDolbyDSProfile = root.findPreference(KEY_DOLBY_DS_PROFILE);
        if (DOLBY_ALLOW_PROFILE_SELECTION) {
            ...
            mDolbyDSProfile.setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
                @Override
                public boolean onPreferenceChange(Preference preference, Object newValue) {
                    if (mDsClientConnected) {
                        updateDolbyStateUI();
    ...
    private void updateDolbyStateUI() {
        if (mDsClient != null && mDsClientConnected) {
                ...
                try {
                    if (mDsClient.getDsOn()) {
                        ...
                        if (profileNames != null && profile >= 0 && profile < profileNames.length) {
                            profileName = mDefaultProfileNames[profile];
                        }

                        // If the custom profile name isn't modified, use the localized string.
                        final Context activity = getActivity();
                        final int cmf = SoundDolbyProfilePreference.getCustomModifyFlag(activity);
                        //Log.d(TAG, "cmf = " + cmf + ", profile = " + profile);
                        if (SoundDolbyProfilePreference.INDEX_CUSTOM1 == profile) {
                            if (SoundDolbyProfilePreference.CUSTOM_1_NAME_MODIFIED == (cmf & SoundDolbyProfilePreference.CUSTOM_1_NAME_MODIFIED)) {
                                profileName = profileNames[profile];
                            }
                        } else if (SoundDolbyProfilePreference.INDEX_CUSTOM2 == profile) {
                            if (SoundDolbyProfilePreference.CUSTOM_2_NAME_MODIFIED == (cmf & SoundDolbyProfilePreference.CUSTOM_2_NAME_MODIFIED)) {
                                profileName = profileNames[profile];
                            }
                        }

                        mDolbyDSProfile.setSummary(profileName);
                        mDolbyLaunchApp.setSummary(profileName);
                    } else {
                        mDolbyDSProfile.setSummary(R.string.sound_dolby_ds_profile_off);
                        mDolbyLaunchApp.setSummary(R.string.sound_dolby_ds_profile_off);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                    unbindDsClient();
            ...
            if (DOLBY_ALLOW_PROFILE_SELECTION) {
                ...
                mDolbyLaunchApp.setSummary(null);
            }
        }
    }
