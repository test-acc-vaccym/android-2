...
/*发短信息*/
function pmwinSendMsg(uid,nickname) {
	window.open("../supesite/newdev.controlpanel.php?type=none&operation=sendmessage&keywords="+uid);
}
...
function ajaxpost_load() {
	...
	try {
		if(is_ie) {
			...
			s = $(ajaxpostHandle[1]).contentWindow.document.documentElement.firstChild.wholeText;
		}
	} catch(e) {		
		if(ajaxdebug) {
...
function floatwin(action, script, w, h, scrollpos) {
	...
	if(handlekey == "login"){  //跳转到社区登录
		if(host != "bbs.mmarket.com:8001"){
			var refererUrl = window.location.href;
			window.location.href = "../supesite/newdev.loginout.php?refererUrl="+escape(refererUrl);
			return;
		}else{
			allowfloatwin = 1;	
		}
	}else if(handlekey == "register"){ //跳转到社区注册
		if(host != "bbs.mmarket.com:8001"){
			window.location.href = "/reg_menu/in.html";
			return;
		}else{
			allowfloatwin = 0;
		}
	}else if(handlekey == "newthread"){
		if(host == "bbs.mmarket.com:8001"){
			allowfloatwin = 1;
		}		
	}
